...
/** Максимум реакций (лайк / полезно / интересно) на одного пользователя в блоге. */
...
const KEYS = {
  ...
  userReactions: 'blogUserReactionsV1',
};
...
function reactionEntryKey(postId, kind) {
  return `${String(postId || '').trim()}:${String(kind || '').trim()}`;
}

function normalizeEngagementReactionKind(op, payload) {
  if (op === 'like') return 'like';
  if (op === 'react') {
    const kind = payload?.kind === 'try' ? 'tryIt' : String(payload?.kind || '');
    return ['useful', 'new', 'tryIt'].includes(kind) ? kind : null;
  }
  return null;
}

async function readAllUserReactions(db) {
  await ensureSeeded(db);
  const map = parseJson(await readSetting(db, KEYS.userReactions), {});
  return map && typeof map === 'object' && !Array.isArray(map) ? map : {};
}

async function getUserReactionList(db, userId) {
  const uid = String(userId || '').trim();
  if (!uid) return [];
  const map = await readAllUserReactions(db);
  const list = map[uid];
...
async function registerUserReaction(db, userId, postId, kind) {
  const uid = String(userId || '').trim();
  const pid = String(postId || '').trim();
  const rk = String(kind || '').trim();
  if (!uid || !pid || !rk) return { ok: false, error: 'Некорректная реакция.' };

  const map = await readAllUserReactions(db);
  const list = Array.isArray(map[uid]) ? [...map[uid]] : [];
  const key = reactionEntryKey(pid, rk);
  if (list.some((row) => reactionEntryKey(row.postId, row.kind) === key)) {
    return { ok: true, list: await getUserReactionList(db, uid), duplicate: true };
  }
  if (list.length >= MAX_USER_REACTIONS) {
    return { ok: false, error: 'Можно поставить не больше 3 реакций в блоге.' };
  }

  list.push({ postId: pid, kind: rk, at: new Date().toISOString() });
  map[uid] = list;
  await writeSetting(db, KEYS.userReactions, map);
  return { ok: true, list: await getUserReactionList(db, uid), duplicate: false };
}
...
async function applyEngagement(db, postId, payload, user) {
  ...
  if (op === 'like' || op === 'react') {
    if (!user?.id) return { error: 'Требуется вход.' };
    const kind = normalizeEngagementReactionKind(op, payload);
    if (!kind) return { error: 'Некорректная реакция.' };
    const reg = await registerUserReaction(db, user.id, id, kind);
    if (!reg.ok) return { error: reg.error };
    myReactions = reg.list;
    if (reg.duplicate) {
      return { ok: true, post, reads: await readReads(db), myReactions, reactionsLimit: MAX_USER_REACTIONS };
    }
  }
  ...
  await writeSetting(db, KEYS.archive, posts);
  const reads = await readReads(db);
  if (user?.id && (op === 'like' || op === 'react')) {
    myReactions = myReactions || (await getUserReactionList(db, user.id));
    return { ok: true, post, reads, myReactions, reactionsLimit: MAX_USER_REACTIONS };
  }
  return { ok: true, post, reads };
}

async function getPublicFeed(db, userId) {
  ...
  const uid = String(userId || '').trim();
  if (uid) {
    out.myReactions = await getUserReactionList(db, uid);
    out.reactionsLimit = MAX_USER_REACTIONS;
  }
  return out;
}
